...
$(document).ready(function () {
    ...
    $('#reportsTable').on('click', '.send-btn', function () {
        ...
        showConfirmModal("Are you sure you want to send this report to the supervisor?", function() {
            sendToSupervisor(userID, reportID); // Call sendToSupervisor when "Yes" is clicked
        });
    });
...
<td>
  ${
                    data.dateCreated
                        ? new Date(data.dateCreated).toISOString().split("T")[0]
                        : "N/A"
                }
</td>                        <td>${username}</td> <!-- Generated By column -->
...
function initializeDataTable() {
    ...
    $('#reportsTable').DataTable({
        dom: 't',          // Only show the table (no pagination, search, etc.)
        paging: false,
        info: false,
        searching: false,
        order: [[2, 'desc']]
        // Disable default DataTables sorting
    });
}
...
async function sendToSupervisor(userID, reportID) {
    ...
    try {
        ...
        usersSnapshot.forEach((childSnapshot) => {
            ...
            console.log(`Checking user: ${childSnapshot.key}, Role: ${userData.role}`);

            if (userData.role == 2) { // Ensure it's a supervisor
                const supervisorID = childSnapshot.key; // Get supervisor UID

                // Reference for supervisorportReports/{supervisorID}/{reportID}
                ...
                set(supervisorRef, {
                    ...
                    console.log(`Report ${reportID} sent to Supervisor: ${supervisorID}`);
                }).catch((error) => {
...
async function updateAuditInFirebase(userID, reportID) {
    ...
    set(portReportRef, {
        ...
        .then(() => {
            console.log("CVE Report audited successfully!");
        })
        .catch((error) => {
            console.error("Error auditing CVE report:", error);
        });
}
